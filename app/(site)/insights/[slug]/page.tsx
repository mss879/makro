...
import { getInsights, getInsightBySlug, getInsightSlugs } from "@/lib/blog-data";
...
export default async function InsightPage({
  ...
  /* Related reading resolves against the list already in hand — hand-picked
     slugs can point at articles the admin has since unpublished, and those
     drop out here exactly as a retired project does below. */
  const related = insight.related.map((s) => allInsights.find((i) => i.slug === s)).filter(Boolean);
  ...
  const projects = relatedProjects.length > 0 ? relatedProjects : allProjects;
